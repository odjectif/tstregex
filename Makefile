...
MAKEMAKER   = /usr/local/share/perl5/site_perl/5.26/ExtUtils/MakeMaker.pm
MM_VERSION  = 7.64
MM_REVISION = 76400
...
pure_all :: $(INST_SCRIPT)/tstregex
	$(NOECHO) $(NOOP)

realclean ::
	$(RM_F) \
	  $(INST_SCRIPT)/tstregex 

$(INST_SCRIPT)/tstregex : tstregex $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/tstregex
	$(CP) tstregex $(INST_SCRIPT)/tstregex
	$(FIXIN) $(INST_SCRIPT)/tstregex
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/tstregex
